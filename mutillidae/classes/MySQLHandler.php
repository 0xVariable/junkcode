...
class MySQLHandler {
	 ...
	 * account root has to match what is in database.config.php.
	...
	private function doOpenDatabaseConnection(){
		...
		$MUTILLIDAE_DBV1_PASSWORD = "";
		$MUTILLIDAE_DBV2_PASSWORD = "mutillidae";
		...
		try{
		    ...
		    if ($lResult <> 1){
		        $lResult = $this->doConnectToDatabase($HOSTNAME, $USERNAME, $MUTILLIDAE_DBV1_PASSWORD);
		        if ($lResult <> 1){
		            $lResult = $this->doConnectToDatabase($HOSTNAME, $USERNAME, $MUTILLIDAE_DBV2_PASSWORD);
		            if ($lResult <> 1){
		                $lResult = $this->doConnectToDatabase($HOSTNAME, $USERNAME, $SAMURAI_WTF_PASSWORD);
		            }//end if
		        }//end if
		    }//end if
